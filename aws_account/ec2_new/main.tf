locals {
  name   = var.name   ### update ec2 machine name
  region = var.region       ## update region in which you need to create ec2 
...
module "ec2_complete" {
  ...
  instance_type               = var.instance_type      ## update instance type
  availability_zone           = data.aws_subnet.selected.availability_zone
  subnet_id                   = data.aws_subnet.selected.id
  vpc_security_group_ids      = [data.aws_security_group.ec2_sg.id]
  #placement_group             = aws_placement_group.web.id
  key_name                    = var.key_name               ## update pem key name
  associate_public_ip_address = var.associate_public_ip_address                     ## update to "true" if you need public IP on ec2 machine
